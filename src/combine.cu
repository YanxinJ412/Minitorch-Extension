...
__global__ void MatrixMultiplyKernel(
  ...
  for (int i = 0; i < ceil(a_shape[2]/double(TILE)); i++) {
  // for (int i = 0; i < a_shape[2]; i++) {
    ...
    if (row < a_shape[1] && a_col < a_shape[2]) {
      ...
      a_shared[threadIdx.y][threadIdx.x] = a_storage[a_position];
    } else {
      a_shared[threadIdx.y][threadIdx.x] = 0.0f;
    }
    if (b_row < b_shape[1] && col < b_shape[2]) {
      int b_position = batch * b_batch_stride + b_row * b_strides[1] + col * b_strides[2];
      b_shared[threadIdx.y][threadIdx.x] = b_storage[b_position];
    } else {
...
__global__ void reduceKernel(
  ...
  float reduce_temp = reduce_value;
  ...
  for (int j = 0; j < a_shape[reduce_dim]; j++) {
    ...
    reduce_temp = fn(fn_id, reduce_temp, a_storage[in_position]);
  }
  // 5. Write the reduced value to out memory
  int out_position = index_to_position(out_index, out_strides, shape_size);
  out[out_position] = reduce_temp;
  // assert(false && "Not Implemented");
  /// END HW1_3
}
