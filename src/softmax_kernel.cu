...
__global__ void ker_attn_softmax(T *inp, const T *attn_mask, int from_len,
  ...
  for (int token_id = blockIdx.x * token_per_reduce; token_id < from_len;
    ...
    for (int i = 0; i < token_per_reduce; i++) {
      ...
      for (int j = 0; j < ele_per_thread; j++) {
        float temp_val;
        int cid = threadIdx.x + j * block_dim;
        if (mask_future && cid > token_id + i) {
          temp_val = REDUCE_FLOAT_INF_NEG;
        } else {
          temp_val = (float)inp_val[i][j];
          if (attn_mask) {
...
void launch_attn_softmax_bw(float *out_grad,
  ...
  // BEGIN ASSIGN4_1_2
  int float_size = sizeof(float);
  int size = rows * softmax_len * float_size;

  // float *d_out_grad, *d_soft_outp;
  // cudaMalloc((void **)&d_out_grad, size);
  // cudaMalloc((void **)&d_soft_outp, size);

  float *buf = nullptr;
  cudaMalloc((void**)&buf, 2 * size);

  float *d_out_grad = buf;
  float *d_soft_outp = buf + rows * softmax_len;

  cudaMemcpy(d_out_grad, out_grad, size, cudaMemcpyHostToDevice);
  cudaMemcpy(d_soft_outp, soft_outp, size, cudaMemcpyHostToDevice);
