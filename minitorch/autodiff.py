...
def topological_sort(variable: Variable) -> Iterable[Variable]:
    ...
    def dfs(var):
        vid = var.unique_id
        if vid in visited or var.is_constant():
            return
        visited.add(vid)
    ...
    dfs(variable)
    order.reverse()
    non_const = [var for var in order if not var.is_constant()]
    return non_const
...
def backpropagate(variable: Variable, deriv: Any) -> None:
    ...
    for var in topo_order:
        vid = var.unique_id
        if var.is_leaf():
            var.accumulate_derivative(grad[vid])
        elif var.is_constant():
            continue
        else:
            for parent, parent_grad in var.chain_rule(grad[vid]):
                pid = parent.unique_id
                grad[pid] = grad.get(pid, 0) + parent_grad
    # raise NotImplementedError("Task Autodiff Not Implemented Yet")
    # END ASSIGN2_1
